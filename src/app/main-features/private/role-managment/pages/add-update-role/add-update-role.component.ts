...
export class AddUpdateRoleComponent implements OnInit{
      ...
      .subscribe({
        next: (result: IRoleAuthority) => {
          ...
          if (this.idEntity() && result.entity?.id) {
            ...
            this.getAllPermissions(result.entity);
          }
  ...
  private getAllPermissions(authority: IAuthority): void{
      ...
      .subscribe({
        next: (result: IRolePermission) => {
          if (result.entities.length === 0 && result.totalPages === -1) {
            ...
            console.log('this.listPermissions ', this.listPermissions())

            authority.permissions?.forEach(value => {
